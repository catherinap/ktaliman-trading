...
# Momentum fields added to SELECT — read directly from DB
NORMALIZED_SELECT = """
SELECT
    ...
    -- Momentum fields (stored in DB by worker)
    funds_index_3w_avg,
    funds_index_8w_avg,
...
def get_assets():
    with engine.connect() as conn:
        ...
        rows = conn.execute(
            text(f"{NORMALIZED_SELECT} WHERE report_date = :d ORDER BY sector, name"),
            {"d": latest_date},
        ).mappings().all()
...
def get_asset(symbol: str):
    with engine.connect() as conn:
        latest = conn.execute(
            text(f"{NORMALIZED_SELECT} WHERE symbol = :s ORDER BY report_date DESC LIMIT 1"),
            {"s": symbol},
        ).mappings().first()
        ...
        history = conn.execute(
            text(f"{NORMALIZED_SELECT} WHERE symbol = :s ORDER BY report_date ASC"),
            {"s": symbol},
        ).mappings().all()
